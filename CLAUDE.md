# SUPER HOT NES

## Project Overview

A SUPERHOT demake for the NES — "time moves when you move" as a side-scrolling platformer. Built with cc65 + neslib targeting MMC1 (Mapper 1) with switchable mirroring.

## Build

```bash
# Requires: cc65 (installed via chocolatey), python3
bash build.sh        # Unix/Git Bash
build.bat            # Windows CMD
```

The build compiles `src/main.c` → assembles with `lib/crt0.s` → links with neslib → outputs `superhotdemake.nes` (40KB: 16B header + 32KB PRG + 8KB CHR).

To regenerate tile graphics: `python tools/gen_chr.py`

## Project Structure

- `src/main.c` — All game logic (single file: tick system, player, enemies, bullets, levels, game flow)
- `lib/` — NES runtime: `crt0.s` (startup), `neslib.h/.sinc` (hardware abstraction), `display.sinc`, `famitone2.sinc` (sound engine, currently unused), `chr.s` (links CHR data)
- `cfg/game.cfg` — Linker memory map (MMC1: 32KB PRG, 8KB CHR, Mapper 1, switchable mirroring)
- `chr/game.chr` — 8KB tile data generated by `tools/gen_chr.py`
- `tools/gen_chr.py` — Generates all sprite and background tiles programmatically
- `tools/mesen/` — Mesen2 emulator for testing
- `sound/` — Reserved for FamiStudio music exports
- `SUPERHOT_NES_DESIGN_DOC.md` — Full game design document

## Architecture

### cc65 Gotchas

- **Global loop variables**: cc65 uses a software stack, so local vars are slow. We use static globals for loop iterators. **Critical**: nested functions must use SEPARATE iterator variables (`ci`, `si`, `di`, `pj`) to avoid clobbering the caller's `i`/`j`. This caused a hard-to-find corruption bug.
- **No inline, C89 only**: No `//` comments in headers, no variable declarations mid-block.
- **`__fastcall__`**: neslib functions use fastcall convention (last param in A/X registers).
- **16-bit coordinates**: World x positions (`px`, `en_x[]`, `bul_x[]`, `pick_x[]`, `part_x[]`) and all Y positions (`py`, `en_y[]`, `bul_y[]`, `pick_y[]`, `part_y[]`, `plat_y[]`, `ground_y_col[]`) are `unsigned int` (16-bit). Y coordinates are 16-bit to support vertical scrolling (elevator level has 5 stories). **Critical**: any function parameter or temp variable that holds a world coordinate must be `unsigned int`, not `unsigned char`, or the high byte is silently truncated. Use `wx` (global `unsigned int`) for temp x calculations instead of `tmp` (which is `unsigned char`). The `60000U` suffix is needed for large unsigned constants on cc65's 16-bit int.

### Core Systems

- **Tick system**: `game_tick` increments only on player input. Enemy AI, bullet movement, and animations only advance when `tick_advance > 0`. Player always responds to input at 60fps.
- **Collision**: AABB hitboxes. `check_solid()` checks against ground columns (`ground_y_col[]`) and platform list (`plat_x/y/w[]`). Player hitbox shrinks when crouching. Offscreen sprites (negative screen coords) are clipped to avoid unsigned char wrapping artifacts and wasted OAM slots.
- **Crouch animation**: Player and enemies have a 3-stage crouch: stand → half-crouch (`SPR_HALFCR_L/R` at scy+4) → full kneel (`SPR_CROUCH_L/R` at scy+8). Player uses `p_crouch_timer` (0–4); enemies use `en_crouch[i]` (0=stand, 5–14=half-crouch, 15+=full). Weapon positions adjust per stage.
- **Physics**: `pvy` is direct pixel velocity (not fixed-point). Gravity accumulates fractionally via `pvy_frac` (adds `GRAVITY_RATE` per tick, +1 to pvy per 16 accumulated).
- **Enemies**: Struct-of-arrays pattern (`en_x[]`, `en_y[]`, `en_type[]`, etc). Types: RUSHER, GUNNER, SHOTGUNNER, THROWER, KATANA_RUSHER. Passive enemies use `en_timer >= 200` to suppress AI until trigger conditions fire. Enemies must have line-of-sight (`en_sight[]` >= `SIGHT_TICKS`) before firing — resets if player leaves their Y range. Rushers only chase within ~400px. Katana rushers jump to platforms to reach the player, perform melee slashes when close, and dodge incoming bullets by crouching (5x crouch speed, hitbox shrinks at half-crouch). On the elevator level, katana rushers won't walk off edges unless the player is on the same floor. Crouched enemies have reduced hitboxes. **Critical**: vertical distance (`ty`) is `unsigned char` — use `ydist` (`unsigned int`) in `update_enemies()` to avoid wrapping on tall levels (e.g., 256px difference wraps to 0).
- **Scrolling**: MMC1 mapper with switchable mirroring via `set_mirroring()`. Horizontal scrolling uses vertical mirroring (`MIRROR_VERTICAL`); vertical scrolling uses horizontal mirroring (`MIRROR_HORIZONTAL`). Camera (`camera_x`, `camera_y`) follows the player, clamped to level bounds. `scroll(camera_x, camera_y)` called each frame. Both nametables are populated for the scroll direction. Single-screen levels set `level_width_px = 256` and camera stays at 0. Sprite screen-x = `world_x - camera_x`; sprite screen-y = `world_y - camera_y`; sprites outside visible range are clipped.
- **Pass-through platforms**: Brown dashed platforms (`BG_FLOOR_PASS`) that the player can jump up through from below. Crouch + A drops the player through. Collision uses `plat_pass[]` flag per platform.
- **Vertical scrolling**: Elevator level uses `camera_y` for vertical scrolling with horizontal mirroring. Level height spans 5 stories. The elevator platform moves vertically and the camera follows.
- **HUD**: Drawn as sprites (not background tiles) so it doesn't scroll with the camera. Weapon icon sprite + ammo shown as bullet dot sprites + level name label (GARAGE/CORRIDOR/DOJO/BAR/OFFICE/ELEVATOR/CEO as packed mini-font sprite tiles). NES 8-sprite-per-scanline limit means HUD elements are spread across two scanline rows.
- **Weapons**: Guns retain when empty (ammo=0) so player can throw them. B button auto-throws empty weapons. Thrown weapons with ammo become ground pickups on wall hit; empty ones vanish. Enemies throw their empty guns when out of ammo, then become rushers. Crouch (down press) to pick up/swap weapons — only triggers on first frame of press, not while held. Katanas cannot be thrown — B always performs a slash with a dedicated horizontal thrust sprite (`SPR_KATANA_H`). Weapon sprites are positioned close to character hands. Shotgun fires a single tall slug (`bul_tall` flag, `SPR_SLUG` sprite rendered as two stacked tiles) with a 16px hitbox that can't be crouched under — forces a jump to dodge.
- **Levels**: 7 levels (0–6) defined procedurally in `init_level()`. Ground height per-column (`ground_y_col[LEVEL_COLS]`, LEVEL_COLS=64) + platform list. `level_width_px` sets the width per level (256 for single-screen, 512 for two-screen). Platforms spaced 5 tile rows apart for jumpability with clearance for bullets underneath.
  - **Level 0 — GARAGE**: Single-screen parking garage. Cars as platforms, concrete pillars, 3 shotgunners + 1 rusher.
  - **Level 1 — CORRIDOR**: 2-screen scrolling hallway. 6 enemies, learn the time mechanic.
  - **Level 2 — DOJO**: 2-screen house structure. Start with katana. 2 gunners + 2 katana rushers.
  - **Level 3 — BAR**: 2-screen bar with glass entrance door (auto-opens on approach), counter, shelves, columns. Back room door opens when 3 of 4 bar enemies are dead. Live VRAM updates via `set_vram_update` buffer for door tile changes. Bar counter and shelf are pass-through but skip brown palette coloring.
  - **Level 4 — OFFICE**: 2-screen office with cubicle dividers. Bottle throwers, gunners, and a katana wielder.
  - **Level 5 — ELEVATOR**: 5 stories with vertical scrolling and horizontal mirroring. 7 enemies including 3 katana rushers. Elevator platform + staircases. Brown pass-through floors.
  - **Level 6 — CEO**: Single-screen corner office. One passive enemy (timer=200). Window on right wall (BG_LGREY). Player wins by walking through the window (px >= 236), not by killing enemies. Right wall collision disabled for this level. "THE END" screen shown after completion.
- **Sprite flipping**: Characters and weapons use `OAM_FLIP_H` for left-facing sprites. For 2x2 character metasprites, the left/right tile columns are swapped and each tile gets the flip attribute.

### CHR Tile Map

Sprite pattern table 0: character metasprites (stand/walk/jump/half-crouch/crouch/punch/kick/slash), bullet, shotgun slug, weapons (pistol/shotgun/katana/katana-horizontal-thrust/bottle), shatter particles, thrown weapon spin, level label tiles (GARAGE/CORRIDOR/DOJO/BAR/OFFICE/ELEVATOR/CEO as packed mini-font), A-Z sprite font, 0-9 sprite digits.
BG pattern table 1: wall/floor/platform tiles, door, bar counter, shelf, column, A-Z font, big SUPERHOT block letters, HUD weapon icons.

Sprites are shared between player (palette 0: white) and enemies (palette 1: red) — same tiles, different palette.

### Audio

- **FamiTone2**: Music engine and SFX system, initialized in crt0.s startup. `FamiToneUpdate` runs every NMI via neslib. SFX enabled (`FT_SFX_ENABLE=1`).
- **Music**: Composed in FamiStudio text format (`sound/superhot_bgm.txt`), exported via CLI: `FamiStudio sound/superhot_bgm.txt famitone2-asm-export sound/music_data.s -famitone2-asm-format:ca65`. Plays on title screen and death; stops during gameplay. Melody transcribed from SUPERHOT:MCD end chiptune via librosa pitch analysis.
- **SFX**: Composed in FamiStudio (`sound/sfx.txt`), exported: `FamiStudio sound/sfx.txt famitone2-asm-sfx-export sound/sfx_data.s -famitone2-asm-format:ca65`. Effects: gunshot (noise ch), jump (pulse rising arp), enemy death (pulse descending). Called via `sfx_play(effect, channel)`.
- **DPCM**: "SUPER HOT" voice clip generated from Windows TTS, converted to NES DPCM via `tools/gen_dpcm.py`. Stored in DPCM segment at $F000. Played by writing APU registers directly ($4010-$4015) during victory screen. Loops with quarter-second gaps.
- **Important**: C pointer writes to APU registers (`*(unsigned char*)0x4000 = val`) don't work reliably in cc65. Use FamiTone2's system or assembly functions for sound. DPCM is the exception — direct writes to $4010-$4015 work.

### Title Screen

Alternates between "SUPER" and "HOT" in big block letters every 0.5 seconds (same effect as between-level screens). Shows "DEMAKE", "AUTHOR: VERI", and "PRESS START". Only redraws big text when phase changes to avoid flicker. Music plays during title screen.

## Testing

Open `superhotdemake.nes` in Mesen2 (`tools/mesen/Mesen.exe`). Use Mesen's debugger (Debug > Debugger) for breakpoints and memory inspection. PPU Viewer shows tile/palette state.

## Design Doc

See `SUPERHOT_NES_DESIGN_DOC.md` for full game design: level layouts, enemy placement, intended player flow, audio design, and NES hardware constraints.
